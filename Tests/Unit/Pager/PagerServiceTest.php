...
namespace ONGR\PagerBundle\Tests\Unit\Pager;

use ONGR\PagerBundle\Pager\PagerService;
use ONGR\PagerBundle\Paging\Adapters\ArrayAdapter;

/**
 * Unit tests for pager service.
 */
class PagerServiceTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $pager = new PagerService(new ArrayAdapter($this->getDummyData()), $options);
    ...
    /**
     * Check if results returned is correct.
     *
     * @param array $options
     ...
     */
    public function testGetResults($options, $expectedData)
    {
        $pager = new PagerService(new ArrayAdapter($this->getDummyData()), $options);

        foreach ($expectedData as $expectedPage) {
            $this->assertEquals($expectedPage, $pager->getResults());
    ...
    {
        $pager = new PagerService(new ArrayAdapter($this->getDummyData()), ['limit' => 3]);
    ...
    {
        $pager = new PagerService(new ArrayAdapter($this->getDummyData()), ['limit' => 6]);
    ...
    {
        $pager = new PagerService(new ArrayAdapter($this->getDummyData()), ['limit' => $limit]);
    ...
    {
        $adapter = new ArrayAdapter([$this->getDummyData()]);
